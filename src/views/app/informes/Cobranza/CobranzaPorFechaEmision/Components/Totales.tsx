  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col font-semibold">
        ...
        <span className="tabular-nums text-base v1440:text-lg font-roboto overflow-x-auto w-full flex flex-row items-center gap-1 justify-start">
...
function TotalBox({ principales, extras, className = "" }: TotalesCobranzaProps) {
  return (
    <Card
      padding={false}
      ...
      {extras && (
        <div className=" grid grid-cols-2 gap-1 px-2 pt-1 v1440:gap-2  v1440:px-4   ">
          {extras?.map((item, index) => (
            <div key={index} className="grid grid-cols-5 col-span-full overflow-x-auto noneScroll items-center border-gray-300  border rounded-md py-1  pl-2  v1440:pl-4 v1440:py-2">
              {/**Texto e icono */}
              <div
                className=" col-span-3 v1536:col-span-3 flex gap-0 w-full justify-start text-xs items-center 
...
export default function Totales() {
  ...
  const {estaProcesado } = useCobranzasPorFechaEmision()

  return <TotalBox className="row-start-3 col-span-8 "
   principales={estaProcesado ? totalesProps.principales : []} extras={estaProcesado ? totalesProps.extras : []} />;
}
