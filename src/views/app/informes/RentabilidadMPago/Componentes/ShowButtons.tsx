...
interface ShowModalButtonsProps {
  propsShowModal: {
    ...
    setShowOpciones: React.Dispatch<React.SetStateAction<boolean>>;
...
export default function ShowButtons({ props, className, estaProcesado }: { props: ShowModalButtonsProps , className?: string, estaProcesado: boolean;}) {
  ...
  const setShowOpciones = propsShowModal?.setShowOpciones;
  ...
  return (
    <div className={`${className} `} >
      ...
      <ActionButton text="Opciones"  color="blue" 
      disabled={!estaProcesado} onClick={() => setShowOpciones(true)}    icon={<IoMdOptions size={12}  />}
      addClassName="h-5 rounded-md text-xxs  v1440:h-8 v1536:h-8 v1536:px-6 v1536:text-sm" />
    </div>
  );
}