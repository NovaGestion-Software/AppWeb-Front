...
type VentasUnidadNegocioProps = {
  ...
  setIdsCoincidentes: (ids: (string | number)[]) => void; // Inputs Radio Filtros
  checkboxSeleccionados: CheckboxState;
  setCheckboxSeleccionados: (grupo: keyof CheckboxState, value: string | null) => void;
};

export const VentasUnidadNegocioStore = create<VentasUnidadNegocioProps>()(
  persist(
    (set) => ({
      id: "",
      setId: (id) => set({ id }),
    ...
    {
      name: "ventas-unidad-negocio-storage",
