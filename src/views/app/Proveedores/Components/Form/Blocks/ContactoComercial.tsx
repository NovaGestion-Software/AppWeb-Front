...
export default function ContactoComercial() {
  const { telefono, codarea, telefono1, codarea1, telefono2, codarea2, email } = useDatosComercialesValues();
  ...
  return (
    <>
      ...
      <Field label="Email" colStart={5} colSpan={4}>
        <FlexibleInputField value={mail.value} disabled={mail.disabled} placeholder="correo@ejemplo.com" labelClassName="hidden" inputClassName={inputsClass} onChange={mail.onChange} onBlur={mail.onBlur} />
        {mail.error && <small className="text-red-500">{mail.error}</small>}
      </Field>

      {/* Teléfonos con código de área (componente hijo) */}
      <TelefonosConCodArea label="Teléfono" inputsClass={inputsClass} base={{ area: area0, tel: tel0 }} v1={{ area: area1, tel: telv1 }} v2={{ area: area2, tel: telv2 }} />
    </>
  );
}
