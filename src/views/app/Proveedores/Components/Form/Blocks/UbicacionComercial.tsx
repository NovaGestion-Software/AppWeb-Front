...
export default function UbicacionComercial() {
  const {
    domicilio1, domicilio2, localidad, cpostal,
    calle1, calle2, idcodprov, latitud, longitud,
  } = useDatosComercialesValues();

  const { setField } = useDatosComercialesActions();

  // Hooks por campo con validación (onBlur)
  ...
  return (
    <>
      ...
      <Field label="Domicilio (2)" colSpan={3} rowStart={2}>
        <FlexibleInputField
          value={dom2.value}
          disabled={dom2.disabled}
          placeholder="Piso, depto, barrio…"
          labelClassName="hidden"
          inputClassName={inputsClass}
          onChange={dom2.onChange}
          onBlur={dom2.onBlur}
        />
